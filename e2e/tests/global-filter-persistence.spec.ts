...
test.describe("Global Filter Persistence - Extended Tests", () => {
  ...
  test("should persist global filter with special characters", async ({ page }) => {
    await page.goto("/");
    ...
    for (const searchTerm of specialSearchTerms) {
      ...
      expect(page.url()).toContain("test-table.search=" + encodeURIComponent(searchTerm));
  ...
  test("should handle global filter with very long search terms", async ({ page }) => {
    await page.goto("/");
    await waitForDataToLoad(page);
  ...
  test("should persist global filter across browser navigation", async ({ page }) => {
    await page.goto("/");
    await waitForDataToLoad(page);
  ...
  test("should handle global filter persistence with mixed case", async ({ page }) => {
    await page.goto("/");
    await waitForDataToLoad(page);
    ...
    for (const term of mixedCaseTerms) {
      ...
      await expect(page.getByTestId("current-state")).toContainText(`Global Filter: ${term}`);
  ...
  test("should persist global filter with rapid typing and debouncing", async ({ page }) => {
    await page.goto("/");
    await waitForDataToLoad(page);
    ...
    // Reload and verify persistence
    await page.reload();
    await expect(page.getByTestId("global-filter")).toHaveValue(finalSearchTerm);
  });

  test("should handle global filter with empty and whitespace values", async ({ page }) => {
    await page.goto("/");
    await waitForDataToLoad(page);
    ...
    await expect(page.getByTestId("current-state")).toContainText("Global Filter:    ");
  ...
  test("should persist global filter when combined with programmatic navigation", async ({ page }) => {
    await page.goto("/");
    await waitForDataToLoad(page);
  ...
  test("should maintain global filter state during error scenarios", async ({ page }) => {
    await page.goto("/");
    await waitForDataToLoad(page);
    ...
    await expect(page.getByTestId("global-filter")).toHaveValue("resilientfilter");
  });

  test("should persist global filter with concurrent user interactions", async ({ page }) => {
    await page.goto("/");
    await waitForDataToLoad(page);
  ...
  test("should handle global filter persistence with URL manipulation", async ({ page }) => {
    await page.goto("/");
    await waitForDataToLoad(page);
    ...
    await expect(page.getByTestId("current-state")).toContainText("Global Filter: manipulated");
  });

  test("should preserve global filter during table refresh operations", async ({ page }) => {
    await page.goto("/");
    await waitForDataToLoad(page);
    ...
    await expect(page.getByTestId("current-state")).toContainText("Global Filter: persistent");
